# Module to for the CDOT API data
module TrafficAlert
  ...
  def self.get_alerts(road)
    if @alert_response
      ...
      @alert_response = HTTParty.get((ENV['CDOT_ALERT_URL']).to_s, basic_auth: @auth)
  ...
  def self.get_images(road)
    if @image_response
      ...
      @image_response = HTTParty.get((ENV['CDOT_IMAGES_URL']).to_s, basic_auth: @auth)
  ...
  def self.get_speeds(road)
    if @speed_response
      ...
      @speed_response = HTTParty.get((ENV['CDOT_SPEEDS_URL']).to_s, basic_auth: @auth)
      get_ski_speeds(@speed_response, road, false) if @speed_response
    end
  end
